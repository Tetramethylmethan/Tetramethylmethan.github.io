var sentencesDe = [
    "Wie heißt Du?",
    "Ich heiße Malte.",
    "Woher kommst Du?",
    "Ich komme aus Berlin.",
    "Wie geht es dir?",
    "Danke gut.",
    "Ich verstehe nicht.",
    "Wir sehen uns!",
    "Ich esse einen Apfel.",
    "Ich gehe zur Schule.",
    "Willst du mit mir spielen?",
    "Was gibt es heute zu essen?",
    "Ich möchte ein Geschenk kaufen.",
    "Sehe ich dich morgen?",
    "Wollen wir zusammen ins Kino gehen?"
]

var sentencesEn = [
    "what is your name ",
    "my name is Malte ",
    "where are you from ",
    "i am from Berlin ",
    "how are you ",
    "i am fine ",
    "i don’t understand ",
    "see you ",
    "i eat an apple ",
    "i go to school ",
    "do you want to play with me ",
    "what is there to eat today ",
    "i want to buy a gift ",
    "will i see you tomorrow ",
    "shall we go to the cinema together "
]

var words = [
    "what", "are", "is", "sun", "your", "name", "there", "a", "fine",
    "my", "an", "from", "name", "fine", "is", "today", "Malte", "apple",
    "where", "come", "understand", "are", "school", "you", "Malte", "from", "see",
    "i", "are", "am", "fine", "from", "apple", "Berlin", "today", "buy",
    "how", "fine", "are", "understand", "you", "from", "want", "to", "play",
    "i", "apple", "am", "today", "fine", "you", "buy", "bus", "talk",
    "i", "don’t", "today", "understand", "am", "car", "go", "tomorrow", "nose",
    "see", "from", "name", "fine", "is", "today", "you", "are", "understand",
    "i", "eat", "understand", "an", "you", "apple", "are", "school",
    "i", "go", "are", "from", "today", "to", "apple", "school", "you",
    "do", "today", "you", "from", "want", "to", "play", "with", "me",
    "what", "are", "is", "fine", "there", "to", "eat", "today", "from",
    "i", "want", "understand", "to", "today", "buy", "a", "gift", "you",
    "will", "apple", "i", "see", "today", "you", "fine", "tomorrow", "there",
    "shall", "fine", "we", "go", "understand", "to", "the", "cinema", "together"
]